...
> **13. Mitigación de saltos y caídas de balance transitorias (2026-06-19)**: Para evitar registrar caídas artificiales del balance y profits erróneos debidos a la latencia post-trade de Binance, `AdjustPositionJob` utiliza el patrimonio neto pre-trade (el último snapshot local `$currentBalance`) como capital de apertura y para el snapshot inicial al abrir posición (LONG/SHORT), eliminando la llamada a `getTotalBalance` post-trade. Al cerrar posiciones (CLOSE) se mantiene la llamada a `getTotalBalance` con espera de `sleep(2)` y hasta 3 reintentos si el balance decae sospechosamente (>5%) para capturar el valor neto final liquidado.
> **12. Profit real al cerrar posición (2026-06-18)**: `AdjustPositionJob` (modo real) ya **no** registra el cierre con un beneficio hardcodeado (+1,5%/+15$). Ahora: al abrir LONG/SHORT guarda el patrimonio neto pre-trade en `Cache user:{id}:open_capital`; al cerrar (señal CLOSE) hace `Cache::pull` de ese capital (fallback al último snapshot) y calcula `profit = equity_cierre − capital_apertura` y `% = profit/apertura×100` en el helper `recordCloseActivity()`, eligiendo `close_profit`/`close_loss` por el signo. El formateador `BotActivityFormatter` para `close_loss` ahora muestra `% y -valor$` con texto neutro en vez del antiguo "protección de pérdida activada".
...
10. **Selector de modo de trading real: Cross Margin vs Futuros (2026-06-18, España/EEE)**:
    ...
    - *Incidencia de Deuda Residual en Cross Margin (Diagnóstico y Solución 2026-06-25)*: Si una posición (p.ej. un SHORT) se cierra con pérdida o si hay deuda de colateral, puede quedar una deuda residual en el activo de margen (USDT/USDC). En Cross Margin, tener deuda activa en el activo de margen reduce su saldo libre (`free`) a 0, haciendo que el bot calcule un tamaño de posición de 0 en las siguientes operaciones y se quede "atascado". **Solución**: Se implementó una autoliquidación en `handleRealCloseMargin` que detecta la deuda residual del activo de margen tras cerrar y llama automáticamente a `repayMarginDebt` (`POST /sapi/v1/margin/borrow-repay`) usando el saldo libre disponible del mismo activo. Si no hay saldo libre de dicho activo, el usuario deberá pagar la deuda manualmente en la interfaz de Binance (opción "Repay").
...
12. **Profit real al cerrar posición (2026-06-18)**:
    ...
    - *Apertura*: Al abrir la posición, el capital de apertura guardado en caché se toma directamente del `$currentBalance` local (pre-trade) para evitar la latencia de balance de Binance.

13. **Mitigación de saltos y caídas transitorias de balance (2026-06-19)**:
    - *Problema*: Al ejecutar operaciones para abrir posición (LONG/SHORT), la llamada a `getTotalBalance` post-trade retornaba balances deflactados (caídas a ~45$ en vez de ~55.8$) debido a la latencia del API de Binance en actualizar el colateral/deuda.
...
15. **Limitación de vinculación por correo (2026-06-19, Actualizado 2026-06-23)**:
    - *Decisión*: Solo los usuarios con los correos electrónicos `vicenteiran@gmail.com` y `julio.vicente.torres@gmail.com` tienen permitido vincular una cuenta de Binance. Si cualquier otro usuario intenta guardar sus credenciales de Binance, la acción es rechazada retornando un error de validación bajo el campo de la API key que indica: "No se ha podido conectar.".
...
16. **El histórico de balance real solo se persiste con datos reales de Binance (Decisión 2026-06-21)**:
    - *Incidencia*: Con `BINANCE_MOCK=true` en prod (Binance estaba IP-restringido), `getTotalBalance` devolvía un valor mock determinista constante (~6855$). El job `binance:sync-balances` (cada ~15 min) y los cierres/aperturas reales lo persistían en `balance_snapshots`, contaminando la curva real (meseta plana en el valor mock que caía al saldo real al desactivar el mock).
    - *Decisión*: **En modo real NO se persiste ningún balance mientras `config('services.binance.mock')` sea `true`.** Todos los escritores de snapshot en real van guardados con `if (! config('services.binance.mock'))`: `AdjustPositionJob` (apertura/cierre real), `DashboardController` (cierre preventivo al pausar), `BalanceController` (fetch inicial), y `SyncBinanceBalances` (cortocircuito al inicio). La simulación NO se toca (sus snapshots siguen). El botón demo "Simular sincronización de balance" (`simulateSync`) queda **restringido a modo simulación**. Los eventos `BalanceUpdated` (UI en vivo) se siguen emitiendo; solo se omite la persistencia.
    - *Efecto*: la curva real refleja solo Binance real y nunca se contamina con datos mock. Si Binance está mal configurado/restringido, simplemente no hay histórico (el header/live sale del saldo en vivo). Para limpiar datos contaminados de una cuenta: `User::where('email',...)->first()->balanceSnapshots()->delete()`.
    - *Tests*: los que verifican persistencia real montan contexto "real" (`config(['services.binance.mock'=>false])` + doble de `BinanceBrokerInterface`, sin red); se añadió `test_sync_command_does_not_persist_when_binance_is_mock`. Ver [[project-architecture]] para el gotcha de `BINANCE_MOCK` en el `.env` de prod.
