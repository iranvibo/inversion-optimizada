...
9. **Soporte para saldo Spot de USDC y Resolución de Doble Contabilidad (2026-07-02 / 2026-07-09)**:
   ...
   - **Doble Contabilidad en Cuentas Unificadas (Corrección 2026-07-09):** En una cuenta unificada (`unifiedAccount` o `portfolioMargin`), la API de Hyperliquid incluye nativamente el saldo de Spot USDC como colateral en el `accountValue` de `clearinghouseState`. Si sumamos `perpBalance` y `spotBalance` (`total - hold`), el saldo libre de USDC en Spot se cuenta doblemente (ej. mostrando $70.88 cuando el patrimonio real es $68.43), ya que la porción de USDC no retenida es parte del colateral y a la vez saldo disponible.
   - **Solución implementada:** Se realiza una consulta al endpoint `userAbstraction` (cacheada por 1 día). Si la cuenta es unificada, `getTotalBalance` devuelve únicamente el saldo total de USDC en Spot (`spotTotal` obtenido de `spotClearinghouseState`), el cual coincide exactamente al centavo con el "Total Equity" y "Trading Equity" visibles en la interfaz de usuario del portfolio de Hyperliquid (antes de restar pérdidas flotantes no realizadas). Si la cuenta es estándar (no unificada), se mantiene la fórmula de suma `perpBalance + spotBalance` ya que ambas carteras son independientes.
