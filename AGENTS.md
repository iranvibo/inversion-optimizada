...
Key constraints   : 
  ...
  - Integration: Binance exchange integration via API. Las señales de trading a ejecutar en Binance se obtienen consultando (polling) una API externa al proyecto, pasándole el nivel de riesgo; responde la posición objetivo (LONG/SHORT/CLOSE) y el histórico de señales para el capital simulado. Un proveedor mock interno (mismo contrato) es el driver por defecto para desarrollo y tests. Existe un segundo canal de ejecución opcional por usuario: Hyperliquid (DEX de perpetuos on-chain, API wallet sin permisos de retiro), seleccionable con users.trading_channel.
